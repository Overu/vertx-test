...
public abstract class BaseTest extends TestCase {
  ...
  protected static class FiveMinuteUser {
    ...
    public boolean equals(Object o) {
      ...
      if (_isVerified != other._isVerified) {
        return false;
      }
      ...
      if (otherImage.length != _userImage.length) {
        return false;
      }
  ...
  protected final static String SAMPLE_DOC_JSON_SPEC = "{\n" + "  \"Image\" : {\n" + "    \"Width\" : " + SAMPLE_SPEC_VALUE_WIDTH + ",\n"
      ...
      + SAMPLE_SPEC_VALUE_TN_ID1 + "," + SAMPLE_SPEC_VALUE_TN_ID2 + "," + SAMPLE_SPEC_VALUE_TN_ID3 + "," + SAMPLE_SPEC_VALUE_TN_ID4 + "]\n"
      + "  }" + "}";
  ...
  protected JsonParser createParserUsingStream(JsonFactory f, String input, String encoding) throws IOException, JsonParseException {
    ...
    /*
     * 23-Apr-2008, tatus: UTF-32 is not supported by JDK, have to use our own codec too (which is not optimal since there's a chance both
     * encoder and decoder might have bugs, but ones that cancel each other out or such)
     */
    byte[] data;
  ...
  /**
   * Method that gets textual contents of the current token using available methods, and ensures results are consistent, before returning
   * them
   */
  protected String getAndVerifyText(JsonParser jp) throws IOException, JsonParseException {
    ...
    if (str.length() != actLen) {
      fail("Internal problem (jp.token == " + jp.getCurrentToken() + "): jp.getText().length() ['" + str + "'] == " + str.length()
          + "; jp.getTextLength() == " + actLen);
    }
